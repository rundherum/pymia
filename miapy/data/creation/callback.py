...
import miapy.data.definition as d
...
class WriteDataCallback(Callback):
    ...
    def on_subject(self, params: dict):
        ...
        self.writer.write('{}/{}'.format(d.DATA_IMAGE, index_str), images)
        if params['has_labels']:
            labels = params['labels']
            self.writer.write('{}/{}'.format(d.DATA_LABEL, index_str), labels)
        if params['has_supplementaries']:
            supplementaries = params['supplementaries']
            for key, value in supplementaries.items():
                self.writer.write('{}/{}/{}'.format(d.DATA, key, index_str), value, value.dtype)
...
class WriteSubjectCallback(Callback):
    ...
    def on_start(self, params: dict):
        ...
        self.writer.reserve(d.SUBJECT, (subject_count,), str)
    ...
    def on_subject(self, params: dict):
        ...
        self.writer.fill(d.SUBJECT, subject, expr.IndexExpression(subject_index))
...
class WriteImageInformationCallback(Callback):
    ...
    def on_start(self, params: dict):
        subject_count = len(params['subject_files'])
        self.writer.reserve(d.INFO_SHAPE, (subject_count, 3), dtype=np.uint8)
        self.writer.reserve(d.INFO_ORIGIN, (subject_count, 3), dtype=np.float)
        self.writer.reserve(d.INFO_DIRECTION, (subject_count, 9), dtype=np.float)
        self.writer.reserve(d.INFO_SPACING, (subject_count, 3), dtype=np.float)

    def on_subject(self, params: dict):
        subject_index = params['subject_index']
        image_properties = params['image_properties']  # type: conv.ImageProperties

        self.writer.fill(d.INFO_SHAPE, image_properties.size, expr.IndexExpression(subject_index))
        self.writer.fill(d.INFO_ORIGIN, image_properties.origin, expr.IndexExpression(subject_index))
        self.writer.fill(d.INFO_DIRECTION, image_properties.direction, expr.IndexExpression(subject_index))
        self.writer.fill(d.INFO_SPACING, image_properties.spacing, expr.IndexExpression(subject_index))
...
class WriteNamesCallback(Callback):
    ...
    def on_start(self, params: dict):
        ...
        self.writer.write(d.NAMES_IMAGE, image_names, dtype='str')

        if params['has_labels']:
            label_names = params['label_names']
            self.writer.write(d.NAMES_LABEL, label_names, dtype='str')

        if params['has_supplementaries']:
            supplementary_names = params['supplementary_names']
            self.writer.write(d.NAMES_SUPPL, supplementary_names, dtype='str')
...
class WriteFilesCallback(Callback):
    ...
    def on_start(self, params: dict):
        ...
        self.writer.write(d.FILES_ROOT, self.file_root, dtype='str')
        image_names = params['image_names']
        self.writer.reserve(d.FILES_IMAGE, (len(subject_files), len(image_names)), dtype='str')

        if params['has_labels']:
            label_names = params['label_names']
            self.writer.reserve(d.FILES_LABEL, (len(subject_files), len(label_names)), dtype='str')

        if params['has_supplementaries']:
            supplementary_names = params['supplementary_names']
            self.writer.reserve(d.FILES_SUPPL, (len(subject_files), len(supplementary_names)), dtype='str')
    ...
    def on_subject(self, params: dict):
        ...
        for image_index, key in enumerate(subject_file.images):
            ...
            self.writer.fill(d.FILES_IMAGE, relative_path, index_expr)
        ...
        for label_index, key in enumerate(subject_file.label_images):
            ...
            self.writer.fill(d.FILES_LABEL, relative_path, index_expr)
        ...
        for supplementary_index, key in enumerate(subject_file.supplementaries):
            ...
            self.writer.fill(d.FILES_SUPPL, relative_path, index_expr)
